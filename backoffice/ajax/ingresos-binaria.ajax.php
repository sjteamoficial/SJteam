...
class TablaIngresos{
	...
	public function mostrarTabla(){
		...
		if($_GET["enlace_afiliado"] != $patrocinador){
			...
			$red = ControladorMultinivel::ctrMostrarUsuarioRed("red_binaria", "usuario_red", $_GET["id_usuario"]);

			$pagos = ControladorMultinivel::ctrMostrarPagosRed("pagos_binaria", "usuario_pago", $_GET["id_usuario"]);
		...
		$periodo_venta = 0;
	 	...
	 	if(count($red) != 0){
			...
			$usuario = ControladorUsuarios::ctrMostrarUsuario("id_usuario", $_GET["id_usuario"]);
